...
def convert_vott_json(name, files, img_path):
    ...
    print(
        f"Attempted {i:g} json imports, found {n1:g} images, imported {n2:g} annotations successfully"
    )
...
def convert_coco_json(
    ...
    for json_file in sorted(Path(json_dir).resolve().glob("*.json")):
        ...
        fn_image = (
            Path(save_dir) / "images" / json_file.stem.replace("instances_", "")
        )  # folder name
        fn.mkdir()
        fn_image.mkdir()
        with open(json_file) as f:
        ...
        for image_ann in images.values():
            shutil.copy(
                Path(images_dir) / image_ann["file_name"],
                fn_image / image_ann["file_name"],
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("-s", "--source", type=str, default="COCO", help="Data source.")
    parser.add_argument("-d", "--data-path", type=str, help="Dataset path.")
    parser.add_argument("-a", "--annotations-path", type=str, help="Annotations path.")
